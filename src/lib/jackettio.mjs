...
function mergeDefaultUserConfig(userConfig) {
  config.immutableUserConfigKeys.forEach((key) => delete userConfig[key]);
  return Object.assign({}, config.defaultUserConfig, userConfig);
}

function prioritizeItems(allItems, prioritizeItems, max) {
  max = max || 0;
  if (typeof (prioritizeItems) === 'function') {
    prioritizeItems = allItems.filter(prioritizeItems);
    if (max > 0) prioritizeItems.splice(max);
  }
  if (prioritizeItems && prioritizeItems.length) {
    allItems = allItems.filter(item => !prioritizeItems.find(i => i === item));
    allItems.unshift(...prioritizeItems);
  }
  return allItems;
}
...
async function getTorrents(userConfig, metaInfos, debridInstance) {
  ...
  try {
    ...
    const filterLanguage = (torrent) => {
      if (prioritizeLanguages.length === 0) return true;
      return torrent.languages.find(lang => ['multi'].concat(prioritizeLanguages).includes(lang.value));
    };
    ...
    if (type === 'movie') {
      ...
      torrents = prioritizeItems(torrents, filterLanguage, Math.max(1, Math.round(maxTorrents * 0.33)));
      ...
      if (prioritizePackTorrents > 0 && packsTorrents.length && !torrents.find(t => packsTorrents.includes(t))) {
        const bestPackTorrents = packsTorrents.slice(0, Math.min(packsTorrents.length, prioritizePackTorrents));
    ...
    if (debridInstance) {
      try {
        ...
        torrents = [].concat(prioritizeItems(cachedTorrents.sort(sortBy(...sortCached)), filterLanguage))
          .concat(prioritizeItems(uncachedTorrents.sort(sortBy(...sortUncached)), filterLanguage));
